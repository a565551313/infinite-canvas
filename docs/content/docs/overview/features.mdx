...
- Keep browser CORS errors away with the same-origin `/api/proxy` relay (Vercel, Cloudflare Pages, Vite dev server) or the local proxy; static hosting without that relay calls providers directly and points to the local proxy when CORS blocks the request.
