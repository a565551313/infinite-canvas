        ...
        <>
            ...
            >
                <span className="relative inline-flex">
                    {APP_VERSION}
                    {version.hasNewVersion ? <span className="absolute -right-1.5 -top-1 size-1.5 rounded-full bg-green-500" /> : null}
                </span>
            </button>
...
type VersionReleaseNotesModalProps = Pick<ReturnType<typeof useVersionCheck>, "open" | "setOpen" | "latestVersion" | "releases" | "checking" | "checkLatestRelease">;

export function VersionReleaseNotesModal({ open, setOpen, latestVersion, releases, checking, checkLatestRelease }: VersionReleaseNotesModalProps) {
    const { t } = useTranslation();
